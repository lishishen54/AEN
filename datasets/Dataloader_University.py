...
class Sampler_University(object):
    ...
    def __init__(self, data_source, batchsize=7,sample_num=7):
...
def train_collate_fn(batch):
    ...
    return [img_s[0].unsqueeze(0),ids[0]],[torch.stack(img_d,dim=0),ids]
...
if __name__ == '__main__':
    ...
    datasets = Dataloader_University(root="E:\\University-Release\\train",transforms=transform_train_list,names=['satellite','drone'])
    samper = Sampler_University(datasets,7)
    dataloader = DataLoader(datasets,batch_size=7,num_workers=0,sampler=samper,collate_fn=train_collate_fn)
    for data_s,data_d in dataloader:                  #每个输出一个包含2个tensor的list，每个tensor中前面那个的尺寸为 batch_size，channel，h，w。即图像矩阵，后面是标签，注意1个batch里面有2种标签的图片
